## Based on the 'nimprof' module built into Nim. Doesn't start profiling
## automatically. Exposes procs to start, stop and report.
...
when defined(profiler):
  ...
  proc writeProfile*(f: File) {.noconv.} =
  ...
  proc writeProfile*(f: File) = discard

  proc disableProfiling*() = discard

  proc enableProfiling*() = discard
